...
import (
	...
	crmmocks "astroapi/internal/clients/retailcrm/mocks"
...
func TestProfileProcessor_HappyPath(t *testing.T) {
	...
	// Mock the initial request retrieval and processing start
	...
	// Mock the natal chart retrieval
	astroProvider.EXPECT().GetNatalChart(gomock.Any(), gomock.Any(), 55.75, 37.61).Return(testNatalData(), nil).Times(1)

	// Mock saving the astro profile
	astroRepo.EXPECT().ExecuteSave(gomock.Any(), gomock.Any()).Return(nil).Times(1)

	// Mock CRM webhook call
	crmRepo.EXPECT().SendProfile(gomock.Any(), gomock.Any()).Return(nil).Times(1)

	// Mock updating status to completed
	reqRepo.EXPECT().UpdateStatus(gomock.Any(), "req-1", requests.StatusCompleted, gomock.AssignableToTypeOf([]byte{}), "").Return(nil).Times(1)

	p := handlers.NewProfileProcessor(astroRepo, reqRepo, astroProvider, crmRepo, zap.NewNop())
